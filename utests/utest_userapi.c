...
START_TEST (test_url_sanity_calendar)
{
	...
	char *tmp = gcal_event_get_url(gcal_event_element(&all_events, 0));
	if (tmp)
	    fail_if(strcmp(gcal_event_get_url(event), tmp) != 0,
		    "Edit url is different!");
	else
	    fail_if(1, "unable to get the URL!");
